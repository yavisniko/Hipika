...
const SetChanges = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/dashboard/getUser/${tokenAuth}/`)
    .then((result) => {
  ...
  useEffect(() => {
    ...
    console.log('is Equal:', showSave)
    console.log('default state', unChanged);
              ...
              <input type="file" accept="image/*" onChange={fileUpload}/>
                {
                  ...
                  : <img src={displayImage.substring(0, 4) !== 'blob' ? `/uploads/avatar/${displayImage}` : displayImage} alt="" />
                }
            </div>
          </div>
          <UserForm showSave={showSave} updateUser={updateUser} inputHandler={inputHandler}/> 
      </div> 
        </>
      }
      </div>
